...
fn bezier_boolean_root_isolation_replay_consumes_algebraic_root_reports() {
    ...
    let construction = match BezierBooleanConstructionReadinessReport2::from_root_isolation_replay(
        scheduler,
        &replay,
        &policy(),
    ) {
...
#[test]
fn bezier_boolean_algebraic_parameter_audit_accepts_ready_events() {
    let range = BezierPathRangeOrderReport2::from_graph_order(
        &BezierMonotoneGraphOrder::IntersectsOrTouches {
            parameters: Vec::new(),
            spans: vec![span(ratio(1, 4), ratio(1, 2))],
...
fn bezier_boolean_algebraic_parameter_audit_blocks_stale_counts_and_roles() {
    ...
    let stale_counts = match handoff.audit(&policy()) {
        Classification::Decided(report) => report,
        Classification::Uncertain(reason) => panic!("unexpected uncertainty: {reason:?}"),
    };
    assert_eq!(
...
#[test]
fn bezier_boolean_operand_locations_expand_non_uniform_locator_outputs() {
    let schedule = BezierBooleanTraversalScheduleReport2 {
        status: BezierBooleanTraversalScheduleStatus::Ready,
        precondition_status: BezierBooleanTraversalPreconditionStatus::Ready,
        first_fragment_count: 2,
    ...
    let report = BezierBooleanOperandOwnershipLocationReport2::from_schedule_locations(
        &schedule,
        &[
            BezierBooleanFragmentOwnershipLocation::Outside,
            BezierBooleanFragmentOwnershipLocation::Inside,
    ...
    assert_eq!(
        report.validate(&schedule).status,
    ...
    let boundary = BezierBooleanOperandOwnershipLocationReport2::from_schedule_locations(
        &schedule,
        &[
            BezierBooleanFragmentOwnershipLocation::Outside,
            BezierBooleanFragmentOwnershipLocation::Boundary,
...
#[test]
fn bezier_boolean_loop_graph_facts_allow_certified_explicit_walks() {
    let plan = BezierBooleanLoopAssemblyPlanReport2 {
        status: BezierBooleanLoopAssemblyPlanStatus::Ready,
        assembly_status: BezierBooleanAssemblyReadinessStatus::Ready,
        operation: BooleanOp::Union,
    ...
    let graph = BezierBooleanLoopGraphFactReport2::from_plan_facts(
        &plan,
        &BezierBooleanLoopGraphFacts2 {
            emitted_step_count: 2,
            branch_vertex_count: 1,
    ...
    assert!(graph.has_blockers());

    let linear = graph.to_traversal_report(&plan);
    assert_eq!(
        linear.status,
        BezierBooleanLoopGraphTraversalStatus::BranchPointsNeedTraversal
    );
    ...
    let certified = graph.to_certified_walk_traversal_report(&plan);
    assert_eq!(
        certified.status,
...
#[test]
fn bezier_boolean_endpoint_successors_derive_exact_reordered_walks() {
    let plan = BezierBooleanLoopAssemblyPlanReport2 {
        status: BezierBooleanLoopAssemblyPlanStatus::Ready,
        assembly_status: BezierBooleanAssemblyReadinessStatus::Ready,
        operation: BooleanOp::Union,
    ...
    let successor = BezierBooleanLoopGraphSuccessorWalkReport2::from_fragment_endpoints(
        &traversal,
        &plan,
        &[
            (point(0, 0), point(1, 0)),
    ...
    let walk = successor.to_graph_walk_report(&traversal, &plan);
    assert_eq!(walk.status, BezierBooleanLoopGraphWalkStatus::Ready);
    assert_eq!(walk.ordered_steps[1].step.fragment_index, 2);
}

#[test]
fn bezier_boolean_endpoint_successors_block_ambiguous_branch_matches() {
    let plan = BezierBooleanLoopAssemblyPlanReport2 {
        status: BezierBooleanLoopAssemblyPlanStatus::Ready,
        assembly_status: BezierBooleanAssemblyReadinessStatus::Ready,
        operation: BooleanOp::Union,
    ...
    let ambiguous = BezierBooleanLoopGraphSuccessorWalkReport2::from_fragment_endpoints(
        &traversal,
        &plan,
        &[
            (point(0, 0), point(1, 0)),
...
fn bezier_boolean_loop_graph_multi_cycle_successor_facts_accept_disjoint_cycles() {
    ...
    let endpoint_multi = BezierBooleanLoopGraphMultiCycleWalkReport2::from_fragment_endpoints(
        &traversal,
        &plan,
        &[
            (point(0, 0), point(1, 0)),
    ...
    assert_eq!(
        endpoint_multi.status,
        BezierBooleanLoopGraphMultiCycleWalkStatus::Ready
    );
    assert_eq!(endpoint_multi.cycle_step_counts, vec![2, 2]);
}
...
#[test]
fn bezier_boolean_loop_containment_queries_cover_ordered_loop_pairs() {
    let plan = BezierBooleanLoopAssemblyPlanReport2 {
        status: BezierBooleanLoopAssemblyPlanStatus::Ready,
        assembly_status: BezierBooleanAssemblyReadinessStatus::Ready,
        operation: BooleanOp::Union,
    ...
    let closure = BezierBooleanLoopClosureReport2::from_fragment_endpoints(
        &plan,
        &[(point(0, 0), point(0, 0)), (point(3, 0), point(3, 0))],
        &[(point(6, 0), point(6, 0))],
    );
    let output = BezierBooleanOutputLoopReport2::from_loop_closure(&closure);
    let locator = BezierBooleanLoopLocatorInputReport2::from_output_loops(&output);
    ...
    assert!(!queries.has_blockers());
    assert_eq!(queries.loop_count, 3);
    assert_eq!(queries.query_count, 6);
    assert_eq!(queries.queries[0].query_loop_index, 0);
    assert_eq!(queries.queries[0].candidate_container_loop_index, 1);
    assert_eq!(queries.queries[0].representative_point, point(0, 0));
    assert!(
        queries
            .queries
            .iter()
            .all(|query| query.query_loop_index != query.candidate_container_loop_index)
    );
}

#[test]
fn bezier_boolean_loop_containment_queries_preserve_locator_blockers_and_singletons() {
    let plan = BezierBooleanLoopAssemblyPlanReport2 {
        status: BezierBooleanLoopAssemblyPlanStatus::Ready,
        assembly_status: BezierBooleanAssemblyReadinessStatus::Ready,
        operation: BooleanOp::Union,
    ...
    let output = BezierBooleanOutputLoopReport2::from_loop_closure(&closure);
    let singleton_locator = BezierBooleanLoopLocatorInputReport2::from_output_loops(&output);
    let singleton =
        BezierBooleanLoopContainmentQueryReport2::from_locator_inputs(&singleton_locator);
    assert_eq!(
        singleton.status,
        BezierBooleanLoopContainmentQueryStatus::NotEnoughLoops
    );
    assert!(singleton.is_vacuously_complete());
    assert!(!singleton.has_blockers());

    let mut malformed_output = output;
    malformed_output.loops[0].directed_fragment_count = 0;
    let blocked_locator =
        BezierBooleanLoopLocatorInputReport2::from_output_loops(&malformed_output);
    let blocked = BezierBooleanLoopContainmentQueryReport2::from_locator_inputs(&blocked_locator);
    assert_eq!(
        blocked.status,
...
#[test]
fn bezier_boolean_graph_walk_depth_role_facts_block_stale_roles() {
    let plan = BezierBooleanLoopAssemblyPlanReport2 {
        status: BezierBooleanLoopAssemblyPlanStatus::Ready,
        assembly_status: BezierBooleanAssemblyReadinessStatus::Ready,
        operation: BooleanOp::Union,
    ...
    let ready = BezierBooleanResultReport2::from_graph_walk_depth_role_facts(
        &walk,
        &plan,
        &endpoints,
        &[],
    ...
    let stale_region = BezierBooleanRegionAssemblyReport2::from_graph_walk_depth_role_facts(
        &walk,
        &plan,
        &endpoints,
        &[],
    ...
    let stale_result = BezierBooleanResultReport2::from_graph_walk_depth_role_facts(
        &walk,
        &plan,
        &endpoints,
        &[],
...
fn bezier_boolean_result_consumes_schedule_multi_cycle_successors_and_depth_facts() {
    ...
    assert_eq!(materialized.component_count, 1);
    assert_eq!(materialized.components[0].material_loop_index, 0);
    ...
    assert_eq!(
        replay_materialized.status,
        BezierBooleanMaterializedRegionStatus::Ready
    );
    assert_eq!(replay_materialized.components[0].hole_loop_indices, vec![1]);
    ...
    assert_eq!(
        BezierBooleanResultReport2::from_quadratic_schedule_multi_cycle_successor_depth_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &quadratic_fragments,
    ...
    assert_eq!(
        BezierBooleanResultReport2::from_cubic_schedule_multi_cycle_successor_depth_role_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &cubic_fragments,
    ...
    assert_eq!(
        BezierBooleanResultReport2::from_rational_quadratic_schedule_multi_cycle_successor_depth_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &rational_fragments,
    ...
    assert_eq!(
        BezierBooleanResultReport2::from_quadratic_schedule_multi_cycle_successor_containment_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &quadratic_fragments,
    ...
    assert_eq!(
        BezierBooleanMaterializedRegionReport2::from_quadratic_schedule_multi_cycle_successor_laminar_containment_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &quadratic_fragments,
    ...
    assert_eq!(
        BezierBooleanResultReport2::from_quadratic_schedule_endpoint_successor_depth_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &quadratic_fragments,
    ...
    assert_eq!(
        BezierBooleanResultReport2::from_cubic_schedule_endpoint_successor_depth_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &cubic_fragments,
    ...
    assert_eq!(
        BezierBooleanResultReport2::from_rational_quadratic_schedule_endpoint_successor_depth_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &rational_fragments,
    ...
    assert_eq!(
        BezierBooleanResultReport2::from_quadratic_schedule_endpoint_successor_containment_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &quadratic_fragments,
    ...
    assert_eq!(
        BezierBooleanResultReport2::from_cubic_schedule_endpoint_successor_containment_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &cubic_fragments,
    ...
    assert_eq!(
        BezierBooleanResultReport2::from_rational_quadratic_schedule_endpoint_successor_containment_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &rational_fragments,
    ...
    assert_eq!(
        BezierBooleanMaterializedRegionReport2::from_quadratic_schedule_endpoint_successor_laminar_containment_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &quadratic_fragments,
    ...
    assert_eq!(
        BezierBooleanMaterializedRegionReport2::from_cubic_schedule_endpoint_successor_laminar_containment_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &cubic_fragments,
    ...
    assert_eq!(
        BezierBooleanMaterializedRegionReport2::from_rational_quadratic_schedule_endpoint_successor_laminar_containment_facts(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &rational_fragments,
    ...
    let typed_containment_role_blocked =
        BezierBooleanResultReport2::from_cubic_schedule_multi_cycle_successor_containment_role_facts(
                &schedule,
                BooleanOp::Union,
                &ownership_facts,
                &cubic_fragments,
    ...
    assert_eq!(
        typed_containment_role_blocked.status,
        BezierBooleanResultStatus::RegionAssemblyBlocked
    );
    assert!(typed_containment_role_blocked.has_blockers());
    assert_eq!(
        BezierBooleanResultReport2::from_rational_quadratic_schedule_multi_cycle_successor_containment_query_results(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &rational_fragments,
    ...
    assert_eq!(
        BezierBooleanMaterializedRegionReport2::from_cubic_schedule_multi_cycle_successor_laminar_containment_query_results(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &cubic_fragments,
    ...
    assert_eq!(
        BezierBooleanResultReport2::from_quadratic_schedule_multi_cycle_successor_containment_certification(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &quadratic_fragments,
    ...
    assert_eq!(
        BezierBooleanMaterializedRegionReport2::from_rational_quadratic_schedule_multi_cycle_successor_laminar_containment_certification(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &rational_fragments,
    ...
    assert_eq!(
        boundary_materialized.status,
        BezierBooleanMaterializedRegionStatus::AuditBlocked
    );
    assert!(boundary_materialized.has_blockers());
...
#[test]
fn bezier_boolean_result_consumes_uniform_graph_fact_identity_depth_facts() {
    let schedule = BezierBooleanTraversalScheduleReport2 {
        status: BezierBooleanTraversalScheduleStatus::Ready,
        precondition_status: BezierBooleanTraversalPreconditionStatus::Ready,
        first_fragment_count: 2,
    ...
    let result = BezierBooleanResultReport2::from_schedule_uniform_graph_fact_identity_depth_facts(
        &schedule,
        BooleanOp::Union,
        BezierBooleanFragmentOwnershipLocation::Outside,
        BezierBooleanFragmentOwnershipLocation::Outside,
    ...
    assert_eq!(
        stale_blocked.status,
        BezierBooleanResultStatus::RegionAssemblyBlocked
    );
    assert!(stale_blocked.has_blockers());
}

#[test]
fn bezier_boolean_result_consumes_operand_locations_graph_fact_identity_depth_facts() {
    let schedule = BezierBooleanTraversalScheduleReport2 {
        status: BezierBooleanTraversalScheduleStatus::Ready,
        precondition_status: BezierBooleanTraversalPreconditionStatus::Ready,
        first_fragment_count: 2,
    ...
    assert_eq!(
        missing_location.status,
        BezierBooleanResultStatus::RegionAssemblyBlocked
    );
    assert!(missing_location.has_blockers());
}

#[test]
fn bezier_boolean_result_consumes_operand_locations_linear_identity_depth_facts() {
    let schedule = BezierBooleanTraversalScheduleReport2 {
        status: BezierBooleanTraversalScheduleStatus::Ready,
        precondition_status: BezierBooleanTraversalPreconditionStatus::Ready,
        first_fragment_count: 2,
...
#[test]
fn bezier_boolean_operand_locations_depth_role_facts_block_stale_roles() {
    let schedule = BezierBooleanTraversalScheduleReport2 {
        status: BezierBooleanTraversalScheduleStatus::Ready,
        precondition_status: BezierBooleanTraversalPreconditionStatus::Ready,
        first_fragment_count: 2,
    ...
    let second_locations = [BezierBooleanFragmentOwnershipLocation::Inside];
    let first_endpoints = [(point(0, 0), point(1, 0)), (point(1, 0), point(0, 0))];
    let second_endpoints = [(point(3, 0), point(3, 0))];
    let depth_facts = [BezierBooleanLoopNestingDepthFact2 {
        loop_index: 0,
        nesting_depth: 0,
    }];

    let ready = BezierBooleanResultReport2::from_schedule_operand_locations_linear_identity_depth_role_facts(
        &schedule,
        BooleanOp::Union,
        &first_locations,
        &second_locations,
    ...
    let stale_identity = BezierBooleanResultReport2::from_schedule_operand_locations_graph_fact_identity_depth_role_facts(
        &schedule,
        BooleanOp::Union,
        &first_locations,
        &second_locations,
    ...
    assert_eq!(
        stale_identity.status,
        BezierBooleanResultStatus::RegionAssemblyBlocked
    );
    assert!(stale_identity.has_blockers());

    let stale_walk =
        BezierBooleanResultReport2::from_schedule_operand_locations_graph_walk_depth_role_facts(
            &schedule,
            BooleanOp::Union,
            &first_locations,
            &second_locations,
    ...
    assert!(stale_walk.has_blockers());
}
...
fn bezier_boolean_result_consumes_operand_locations_linear_identity_containment_facts() {
    ...
    let result = BezierBooleanResultReport2::from_schedule_operand_locations_linear_identity_containment_facts(
        &schedule,
        BooleanOp::Union,
        &[
            BezierBooleanFragmentOwnershipLocation::Outside,
    ...
    let missing_location = BezierBooleanResultReport2::from_schedule_operand_locations_linear_identity_containment_facts(
        &schedule,
        BooleanOp::Union,
        &[BezierBooleanFragmentOwnershipLocation::Outside],
        &[BezierBooleanFragmentOwnershipLocation::Inside],
...
#[test]
fn bezier_boolean_result_consumes_operand_locations_graph_fact_identity_containment_facts() {
    let schedule = BezierBooleanTraversalScheduleReport2 {
        status: BezierBooleanTraversalScheduleStatus::Ready,
        precondition_status: BezierBooleanTraversalPreconditionStatus::Ready,
        first_fragment_count: 2,
    ...
    assert_eq!(
        stale_containment.status,
        BezierBooleanResultStatus::RegionAssemblyBlocked
    );
    assert!(stale_containment.has_blockers());
}

#[test]
fn bezier_boolean_result_consumes_operand_locations_graph_fact_walk_depth_facts() {
    let schedule = BezierBooleanTraversalScheduleReport2 {
        status: BezierBooleanTraversalScheduleStatus::Ready,
        precondition_status: BezierBooleanTraversalPreconditionStatus::Ready,
        first_fragment_count: 2,
    ...
    assert_eq!(result.assigned_loop_count, 2);
    assert_eq!(result.directed_fragment_count, 2);
    assert_eq!(result.material_loop_indices, vec![0, 1]);
    assert_eq!(result.directed_fragments[0].start, point(1, 0));
    ...
    assert_eq!(
        duplicate_walk.status,
        BezierBooleanResultStatus::RegionAssemblyBlocked
    );
    assert!(duplicate_walk.has_blockers());
}

#[test]
fn bezier_boolean_result_consumes_operand_locations_raw_graph_walk_depth_facts() {
    let schedule = BezierBooleanTraversalScheduleReport2 {
        status: BezierBooleanTraversalScheduleStatus::Ready,
        precondition_status: BezierBooleanTraversalPreconditionStatus::Ready,
        first_fragment_count: 2,
    ...
    let result = BezierBooleanResultReport2::from_schedule_operand_locations_graph_walk_depth_facts(
        &schedule,
        BooleanOp::Union,
        &[
            BezierBooleanFragmentOwnershipLocation::Outside,
...
#[test]
fn bezier_boolean_result_accepts_certified_walk_for_branch_graph_facts() {
    let schedule = BezierBooleanTraversalScheduleReport2 {
        status: BezierBooleanTraversalScheduleStatus::Ready,
        precondition_status: BezierBooleanTraversalPreconditionStatus::Ready,
        first_fragment_count: 2,
    ...
    assert_eq!(
        identity_blocked.status,
        BezierBooleanResultStatus::RegionAssemblyBlocked
    );
    assert!(identity_blocked.has_blockers());
}

#[test]
fn bezier_boolean_result_consumes_operand_locations_graph_fact_walk_containment_facts() {
    let schedule = BezierBooleanTraversalScheduleReport2 {
        status: BezierBooleanTraversalScheduleStatus::Ready,
        precondition_status: BezierBooleanTraversalPreconditionStatus::Ready,
        first_fragment_count: 2,
...
#[test]
fn bezier_boolean_result_consumes_operand_locations_raw_graph_walk_containment_facts() {
    let schedule = BezierBooleanTraversalScheduleReport2 {
        status: BezierBooleanTraversalScheduleStatus::Ready,
        precondition_status: BezierBooleanTraversalPreconditionStatus::Ready,
        first_fragment_count: 2,
    ...
    let result =
        BezierBooleanResultReport2::from_schedule_operand_locations_graph_walk_containment_facts(
            &schedule,
            BooleanOp::Union,
            &[
                BezierBooleanFragmentOwnershipLocation::Outside,
...
fn bezier_boolean_result_consumes_containment_query_results_without_dropping_blockers() {
    ...
    let locator_result = BezierBooleanResultReport2::from_schedule_graph_walk_locator_results(
        &schedule,
        BooleanOp::Union,
        &ownership_facts,
        &endpoints,
    ...
    let locator_materialized =
        BezierBooleanMaterializedRegionReport2::from_schedule_graph_walk_laminar_locator_results(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
            &endpoints,
    ...
    assert!(boundary_certification.has_blockers());
    let boundary_scheduled_certification =
        BezierBooleanLoopContainmentCertificationReport2::from_schedule_graph_walk_query_results(
            &schedule,
            BooleanOp::Union,
            &ownership_facts,
    ...
    assert_eq!(boundary_scheduled_certification, boundary_certification);
    ...
    let locator_blocked = BezierBooleanResultReport2::from_schedule_graph_walk_locator_results(
        &schedule,
        BooleanOp::Union,
        &ownership_facts,
        &endpoints,
    ...
    assert_eq!(locator_blocked, certified_blocked);
}
...
#[test]
fn bezier_boolean_materialization_audit_blocks_stale_result_payloads() {
    let schedule = BezierBooleanTraversalScheduleReport2 {
        status: BezierBooleanTraversalScheduleStatus::Ready,
        precondition_status: BezierBooleanTraversalPreconditionStatus::Ready,
        first_fragment_count: 2,
    ...
    assert_eq!(audit.status, BezierBooleanMaterializationAuditStatus::Ready);
    assert!(audit.is_ready());
    assert!(!audit.has_blockers());

    let mut stale_index = result.clone();
    stale_index.material_loop_indices = vec![0, 0];
    stale_index.hole_loop_indices.clear();
    stale_index.material_loop_count = 2;
    stale_index.hole_loop_count = 0;
    let stale_index_audit = BezierBooleanMaterializationAuditReport2::from_result(&stale_index);
    assert_eq!(
        stale_index_audit.status,
        BezierBooleanMaterializationAuditStatus::RoleIndexMismatch
    );
    assert!(stale_index_audit.has_blockers());

    let mut stale_role = result.clone();
    stale_role.material_loop_indices = vec![1];
    stale_role.hole_loop_indices = vec![0];
    let stale_role_audit = BezierBooleanMaterializationAuditReport2::from_result(&stale_role);
    assert_eq!(
        stale_role_audit.status,
        BezierBooleanMaterializationAuditStatus::LoopRoleMismatch
    );
    assert!(stale_role_audit.has_blockers());

    let mut stale_range = result.clone();
    stale_range.assigned_loops[0]
        .output_loop
        .directed_fragment_count = usize::MAX;
    let stale_range_audit = BezierBooleanMaterializationAuditReport2::from_result(&stale_range);
    assert_eq!(
        stale_range_audit.status,
        BezierBooleanMaterializationAuditStatus::FragmentRangeMismatch
    );
    assert!(stale_range_audit.has_blockers());
}
...
#[test]
fn bezier_boolean_laminar_materialized_region_uses_nearest_material_ancestor() {
    let mut emitted_steps = Vec::new();
    let mut endpoints = Vec::new();
    for index in 0..4 {
        let x = index * 3;
    ...
    let cycle = BezierBooleanMaterializedRegionReport2::from_result_laminar_containment_facts(
        &result,
        &[
            BezierBooleanLoopContainmentFact2 {
                container_loop_index: 0,
...
proptest! {
    ...
    fn generated_bezier_boolean_algebraic_parameter_handoff_accepts_interval_roots(
        ...
        let audit = match handoff.audit(&policy()) {
            Classification::Decided(report) => report,
            Classification::Uncertain(reason) => {
                prop_assert!(false, "unexpected audit uncertainty: {reason:?}");
                return Ok(());
            }
        };
        prop_assert_eq!(
    ...
    #[test]
    fn generated_bezier_boolean_operand_locations_expand_schedule_order(
        first_count in 1_usize..8,
        second_count in 1_usize..8,
    ) {
        let overlaps = BezierBooleanOverlapResolutionReport2::from_overlap_events(&[], &policy())
        ...
        let schedule = BezierBooleanTraversalScheduleReport2::from_preconditions(&preconditions);
        let first_locations = (0..first_count)
            .map(|index| {
                if index % 2 == 0 {
                    BezierBooleanFragmentOwnershipLocation::Outside
                } else {
            ...
            .collect::<Vec<_>>();
        let second_locations = (0..second_count)
            .map(|index| {
                if index % 2 == 0 {
                    BezierBooleanFragmentOwnershipLocation::Inside
                } else {
        ...
        for fact in &report.facts {
            let expected = match fact.step.operand {
                BezierBooleanTraversalOperand::First => first_locations[fact.step.fragment_index],
                BezierBooleanTraversalOperand::Second => second_locations[fact.step.fragment_index],
            };
    ...
    #[test]
    fn generated_bezier_boolean_loop_containment_queries_cover_all_ordered_pairs(
        loop_count in 2_usize..7,
    ) {
        let mut emitted_steps = Vec::new();
        let mut endpoints = Vec::new();
        for index in 0..loop_count {
            let x = (index as i32) * 7;
            emitted_steps.push(hypercurve::BezierBooleanOwnedTraversalStep2 {
                step: hypercurve::BezierBooleanTraversalStep2 {
                    operand: BezierBooleanTraversalOperand::First,
                    fragment_index: index * 2,
        ...
        prop_assert_eq!(queries.loop_count, loop_count);
        prop_assert_eq!(queries.query_count, loop_count * (loop_count - 1));
        prop_assert_eq!(queries.queries.len(), queries.query_count);
        prop_assert!(queries
            .queries
            .iter()
            .all(|query| query.query_loop_index != query.candidate_container_loop_index));
    ...
    fn generated_bezier_boolean_loop_containment_query_results_replay_strict_contains(
        ...
        let schedule = BezierBooleanTraversalScheduleReport2 {
            ...
            second_fragment_count: 0,
            steps: ownership_facts.iter().map(|fact| fact.step.clone()).collect(),
            resolved_overlap_count: 0,
            overlap_boundary_parameter_count: 0,
            blocker_count: 0,
        };
        ...
        prop_assert_eq!(scheduled_certification, certification.clone());
    ...
    #[test]
    fn generated_bezier_boolean_result_accepts_certified_walk_for_graph_obligations(
        loop_count in 1_usize..6,
    ) {
        let mut steps = Vec::new();
        let mut endpoints = Vec::new();
    ...
    #[test]
    fn generated_bezier_boolean_result_consumes_uniform_graph_fact_identity_depths(
        loop_count in 1_usize..6,
    ) {
        let mut steps = Vec::new();
        let mut endpoints = Vec::new();
    ...
    #[test]
    fn generated_bezier_boolean_result_consumes_operand_locations_identity_depths(
        loop_count in 1_usize..6,
    ) {
        let mut steps = Vec::new();
        ...
        for index in 0..loop_count {
            ...
            first_endpoints.push((point(x, 0), point(x + 1, 0)));
            steps.push(hypercurve::BezierBooleanTraversalStep2 {
                operand: BezierBooleanTraversalOperand::First,
                fragment_index: index * 2 + 1,
            });
            first_endpoints.push((point(x + 1, 0), point(x, 0)));
        }
        let schedule = BezierBooleanTraversalScheduleReport2 {
            status: BezierBooleanTraversalScheduleStatus::Ready,
            precondition_status: BezierBooleanTraversalPreconditionStatus::Ready,
        ...
        let first_locations = vec![BezierBooleanFragmentOwnershipLocation::Outside; loop_count * 2];
        let depth_facts = (0..loop_count)
            .map(|loop_index| BezierBooleanLoopNestingDepthFact2 {
                loop_index,
                nesting_depth: loop_index * 2,
        ...
        let result = BezierBooleanResultReport2::from_schedule_operand_locations_graph_fact_identity_depth_facts(
            &schedule,
            BooleanOp::Union,
            &first_locations,
            &[],
    ...
    #[test]
    fn generated_bezier_boolean_result_consumes_operand_locations_linear_depths(
        loop_count in 1_usize..6,
    ) {
        let mut steps = Vec::new();
        let mut first_endpoints = Vec::new();
        ...
        let result = BezierBooleanResultReport2::from_schedule_operand_locations_linear_identity_depth_facts(
            &schedule,
            BooleanOp::Union,
            &first_locations,
            &[],
    ...
    #[test]
    fn generated_bezier_boolean_result_consumes_operand_locations_linear_containment(
        loop_count in 1_usize..6,
    ) {
        let mut steps = Vec::new();
        let mut first_endpoints = Vec::new();
    ...
    #[test]
    fn generated_bezier_boolean_result_consumes_operand_locations_identity_containment(
        loop_count in 1_usize..6,
    ) {
        let mut steps = Vec::new();
        let mut first_endpoints = Vec::new();
        ...
        let result = BezierBooleanResultReport2::from_schedule_operand_locations_graph_fact_identity_containment_facts(
            &schedule,
            BooleanOp::Union,
            &first_locations,
            &[],
    ...
    #[test]
    fn generated_bezier_boolean_result_consumes_operand_locations_graph_walk_depths(
        loop_count in 1_usize..6,
    ) {
        let mut steps = Vec::new();
        let mut first_endpoints = Vec::new();
        ...
        let first_locations = vec![BezierBooleanFragmentOwnershipLocation::Outside; loop_count * 2];
        let mut walk_indices = (0..loop_count * 2).collect::<Vec<_>>();
        ...
        let result = BezierBooleanResultReport2::from_schedule_operand_locations_graph_fact_walk_depth_facts(
            &schedule,
            BooleanOp::Union,
            &first_locations,
            &[],
    ...
    #[test]
    fn generated_bezier_boolean_result_consumes_operand_locations_raw_graph_walk_depths(
        loop_count in 1_usize..6,
    ) {
        let mut steps = Vec::new();
        let mut first_endpoints = Vec::new();
    ...
    #[test]
    fn generated_bezier_boolean_result_consumes_operand_locations_graph_walk_containment(
        loop_count in 1_usize..6,
    ) {
        let mut steps = Vec::new();
        let mut first_endpoints = Vec::new();
        ...
        let result = BezierBooleanResultReport2::from_schedule_operand_locations_graph_fact_walk_containment_facts(
            &schedule,
            BooleanOp::Union,
            &first_locations,
            &[],
    ...
    #[test]
    fn generated_bezier_boolean_operand_location_depth_role_paths_reject_wrong_parity(
        loop_count in 1_usize..6,
        depth_offset in 0_usize..8,
        flip_index in 0_usize..6,
    ) {
        ...
        let first_locations = vec![BezierBooleanFragmentOwnershipLocation::Outside; loop_count];
        let second_locations = Vec::new();
        let endpoints = (0..loop_count)
            .map(|index| {
                let x = (index as i32) * 3;
                (point(x, 0), point(x, 0))
        ...
        let ready = BezierBooleanResultReport2::from_schedule_operand_locations_graph_fact_walk_depth_role_facts(
            &schedule,
            BooleanOp::Union,
            &first_locations,
            &second_locations,
        ...
        let stale = BezierBooleanResultReport2::from_schedule_operand_locations_graph_walk_depth_role_facts(
            &schedule,
            BooleanOp::Union,
            &first_locations,
            &second_locations,
    ...
    #[test]
    fn generated_bezier_boolean_materialization_audit_preserves_ready_payloads(
        loop_count in 1_usize..6,
        material_prefix in 1_usize..6,
    ) {
        let mut emitted_steps = Vec::new();
        let mut endpoints = Vec::new();
        for index in 0..loop_count {
        ...
        let output = BezierBooleanOutputLoopReport2::from_loop_closure(&closure);
        let material_count = material_prefix.min(loop_count).max(1);
        let roles = (0..loop_count)
            .map(|index| {
        ...
        prop_assert_eq!(audit.status, BezierBooleanMaterializationAuditStatus::Ready);
        prop_assert_eq!(audit.assigned_loop_count, loop_count);
        prop_assert_eq!(audit.material_loop_count, material_count);
        prop_assert_eq!(audit.hole_loop_count, loop_count - material_count);
        prop_assert_eq!(audit.directed_fragment_count, loop_count * 2);
    ...
    #[test]
    fn generated_bezier_boolean_laminar_materialized_region_accepts_alternating_chain(
        loop_count in 1_usize..8,
    ) {
        let mut emitted_steps = Vec::new();
        let mut endpoints = Vec::new();
