...
use hypercurve::{
    ...
    BezierBooleanAlgebraicParameterHandoffReport2, BezierBooleanAlgebraicParameterOrderingReport2,
    ...
    BezierBooleanOutputLoopRole, BezierBooleanOverlapBridgeFact2,
    BezierBooleanOverlapBridgeReport2, BezierBooleanOverlapResolutionReport2,
...
fn bench_bezier_topology(quadratic: &QuadraticBezier2, cubic: &CubicBezier2) {
    ...
    for _ in 0..iterations {
        ...
        let boolean_root_isolation_handoff =
            BezierBooleanRootIsolationHandoffReport2::from_handoff_reports(&[
                boolean_handoff_from_regions.clone(),
                boolean_handoff_from_certificate.clone(),
            ]);
        let boolean_batch_root_isolation_handoff =
            BezierBooleanRootIsolationHandoffReport2::from_batch_handoff(&boolean_handoff_batch);
        ...
        let boolean_scheduler_root_isolation_handoff =
            BezierBooleanRootIsolationHandoffReport2::from_path_scheduler(&boolean_path_scheduler);
        let boolean_scheduler_root_isolation_replay =
            BezierBooleanRootIsolationReplayReport2::from_hypersolve_roots(
                &boolean_path_scheduler,
                &[],
                &[],
                &policy,
            );
        let boolean_scheduler_hypersolve_report_replay =
            BezierBooleanRootIsolationReplayReport2::from_hypersolve_range_reports(
                &boolean_path_scheduler,
                &[],
                &[],
                &policy,
            );
        let boolean_scheduler_bernstein_replay =
            BezierBooleanRootIsolationReplayReport2::from_hypersolve_bernstein_subdivision_reports(
                &boolean_path_scheduler,
                &[],
        ...
        let boolean_scheduler_algebraic_replay =
            BezierBooleanRootIsolationReplayReport2::from_hypersolve_algebraic_root_reports(
                &boolean_path_scheduler,
                &[],
        ...
        let boolean_scheduler_algebraic_parameter_handoff =
            BezierBooleanAlgebraicParameterHandoffReport2::from_hypersolve_algebraic_root_reports(
                &boolean_path_scheduler,
                &[],
                &policy,
            );
        let boolean_scheduler_algebraic_parameter_audit =
            match &boolean_scheduler_algebraic_parameter_handoff {
                hypercurve::Classification::Decided(handoff) => handoff.audit(&policy),
                hypercurve::Classification::Uncertain(reason) => {
                    hypercurve::Classification::Uncertain(*reason)
                }
            };
        let boolean_scheduler_algebraic_parameter_audit_status =
            match &boolean_scheduler_algebraic_parameter_audit {
                hypercurve::Classification::Decided(audit) => audit.status,
                hypercurve::Classification::Uncertain(_) => {
                    BezierBooleanAlgebraicParameterAuditStatus::HandoffBlocked
                }
            };
        let boolean_scheduler_algebraic_parameter_readiness =
            match &boolean_scheduler_algebraic_parameter_handoff {
                hypercurve::Classification::Decided(handoff) => {
                    BezierBooleanAlgebraicParameterReadinessReport2::from_handoff(handoff, &policy)
                }
                hypercurve::Classification::Uncertain(reason) => {
                    hypercurve::Classification::Uncertain(*reason)
                }
            };
        let boolean_scheduler_algebraic_parameter_ordering =
            match &boolean_scheduler_algebraic_parameter_readiness {
                hypercurve::Classification::Decided(readiness) => {
                    BezierBooleanAlgebraicParameterOrderingReport2::from_readiness(
                        readiness,
                ...
                hypercurve::Classification::Uncertain(reason) => {
                    BezierBooleanAlgebraicParameterOrderingReport2 {
                        status: hypercurve::BezierBooleanAlgebraicParameterOrderingStatus::ReadinessBlocked,
                        readiness_status: hypercurve::BezierBooleanAlgebraicParameterReadinessStatus::AuditBlocked,
                        events: Vec::new(),
                        ...
                        blocker_count: usize::from(matches!(reason, hypercurve::UncertaintyReason::Ordering)),
                    }
                }
            };
        let boolean_scheduler_algebraic_split_bridge =
            match &boolean_scheduler_algebraic_parameter_ordering {
                ordering => {
                    BezierBooleanAlgebraicSplitBridgeReport2::from_ordering(ordering, &policy)
                }
            };
        let boolean_scheduler_algebraic_parameter_carrier =
            BezierBooleanAlgebraicParameterCarrierReport2::from_ordering(
                &boolean_scheduler_algebraic_parameter_ordering,
            );
        let boolean_scheduler_algebraic_bridge_readiness =
            match &boolean_scheduler_algebraic_split_bridge {
                hypercurve::Classification::Decided(bridge) => {
                    BezierBooleanConstructionReadinessReport2::from_algebraic_split_bridge(
                        bridge, &policy,
                    )
                }
                hypercurve::Classification::Uncertain(reason) => {
        ...
        let boolean_scheduler_hypersolve_replay_readiness =
            match &boolean_scheduler_hypersolve_report_replay {
                hypercurve::Classification::Decided(replay) => {
                    BezierBooleanConstructionReadinessReport2::from_root_isolation_replay(
        ...
        let boolean_scheduler_bernstein_replay_readiness = match &boolean_scheduler_bernstein_replay
        {
            hypercurve::Classification::Decided(replay) => {
                BezierBooleanConstructionReadinessReport2::from_root_isolation_replay(
                    boolean_path_scheduler.clone(),
        ...
        let boolean_scheduler_algebraic_replay_readiness = match &boolean_scheduler_algebraic_replay
        {
            hypercurve::Classification::Decided(replay) => {
                BezierBooleanConstructionReadinessReport2::from_root_isolation_replay(
                    boolean_path_scheduler.clone(),
        ...
        let boolean_algebraic_shared_range_fragments =
            match boolean_scheduler_algebraic_bridge_readiness.clone() {
                hypercurve::Classification::Decided(readiness) => {
                    BezierBooleanQuadraticFragmentReport2::from_shared_range_readiness(
                        quadratic, &readiness, &policy,
                    )
                }
                hypercurve::Classification::Uncertain(reason) => {
        ...
        let boolean_fragment_locator_inputs = match (
            &boolean_quadratic_fragments,
            &boolean_quadratic_fragments,
            &boolean_overlap_resolution,
        ) {
            (
                ...
                format!(
                    "{:?}",
                    BezierBooleanFragmentLocatorInputReport2::from_quadratic_schedule_midpoints(
                        &schedule,
                        BooleanOp::Union,
                        first,
                        second,
        ...
        let boolean_loop_graph_successor_walk = match (
            &boolean_quadratic_fragments,
            &boolean_quadratic_fragments,
            &boolean_overlap_resolution,
        ) {
            (
                ...
                let successors = (0..plan.emitted_steps.len())
                ...
                let overlap_bridge_report =
                    BezierBooleanOverlapBridgeReport2::from_fragment_endpoint_tangents(
                ...
                let endpoint_result =
                    hypercurve::BezierBooleanResultReport2::from_schedule_endpoint_successor_depth_facts(
                        &schedule,
                        BooleanOp::Union,
                        &ownership_facts,
                        &first_endpoints,
                ...
                let endpoint_containment_result =
                    hypercurve::BezierBooleanResultReport2::from_schedule_endpoint_successor_containment_facts(
                        &schedule,
                        BooleanOp::Union,
                        &ownership_facts,
                        &first_endpoints,
                ...
                let endpoint_materialized =
                    hypercurve::BezierBooleanMaterializedRegionReport2::from_schedule_endpoint_successor_laminar_containment_facts(
                        &schedule,
                        BooleanOp::Union,
                        &ownership_facts,
                        &first_endpoints,
                ...
                let locator_certification =
                    hypercurve::BezierBooleanLoopContainmentCertificationReport2::from_schedule_multi_cycle_successor_query_results(
                        &schedule,
                        BooleanOp::Union,
                        &ownership_facts,
                        &first_endpoints,
                ...
                let locator_result =
                    hypercurve::BezierBooleanResultReport2::from_schedule_multi_cycle_successor_locator_results(
                        &schedule,
                        BooleanOp::Union,
                        &ownership_facts,
                        &first_endpoints,
                ...
                let locator_materialized =
                    hypercurve::BezierBooleanMaterializedRegionReport2::from_schedule_multi_cycle_successor_laminar_locator_results(
                        &schedule,
                        BooleanOp::Union,
                        &ownership_facts,
                        &first_endpoints,
        ...
        let boolean_loop_locator_inputs = match (
            &boolean_quadratic_fragments,
            &boolean_quadratic_fragments,
            &boolean_overlap_resolution,
        ) {
            (
                ...
                format!(
                    "{:?}",
                    BezierBooleanLoopLocatorInputReport2::from_output_loops(&output)
                )
            }
            other => format!("{other:?}"),
        };
        let boolean_loop_containment_queries = match (
            &boolean_quadratic_fragments,
            &boolean_quadratic_fragments,
            &boolean_overlap_resolution,
        ) {
        ...
        let boolean_schedule_graph_fact_containment_result = match (
            ...
            (
                ...
                let materialized =
                    BezierBooleanMaterializedRegionReport2::from_result_containment_facts(
                        &role_result,
                        &containment_facts,
                    );
        ...
        let boolean_uniform_graph_fact_identity_depth_result = match (
            &boolean_quadratic_fragments,
            &boolean_quadratic_fragments,
            &boolean_overlap_resolution,
        ) {
        ...
        let boolean_operand_location_identity_depth_result = match (
            &boolean_quadratic_fragments,
            &boolean_quadratic_fragments,
            &boolean_overlap_resolution,
        ) {
            (
                ...
                let first_locations = vec![
                    BezierBooleanFragmentOwnershipLocation::Outside;
                    schedule.first_fragment_count
                ];
                let second_locations = vec![
                    BezierBooleanFragmentOwnershipLocation::Outside;
        ...
        let boolean_operand_location_linear_depth_result = match (
            &boolean_quadratic_fragments,
            &boolean_quadratic_fragments,
            &boolean_overlap_resolution,
        ) {
        ...
        let boolean_operand_location_linear_containment_result = match (
            &boolean_quadratic_fragments,
            &boolean_quadratic_fragments,
            &boolean_overlap_resolution,
        ) {
        ...
        let boolean_operand_location_graph_walk_depth_result = match (
            &boolean_quadratic_fragments,
            &boolean_quadratic_fragments,
            &boolean_overlap_resolution,
        ) {
        ...
        let boolean_operand_location_raw_graph_walk_depth_result = match (
            &boolean_quadratic_fragments,
            &boolean_quadratic_fragments,
            &boolean_overlap_resolution,
        ) {
        ...
        let boolean_operand_location_identity_containment_result = match (
            &boolean_quadratic_fragments,
            &boolean_quadratic_fragments,
            &boolean_overlap_resolution,
        ) {
        ...
        let boolean_operand_location_graph_walk_containment_result = match (
            &boolean_quadratic_fragments,
            &boolean_quadratic_fragments,
            &boolean_overlap_resolution,
        ) {
        ...
        let boolean_uniform_linear_identity_depth_result = match (
            &boolean_quadratic_fragments,
            &boolean_quadratic_fragments,
            &boolean_overlap_resolution,
        ) {
        ...
        let boolean_query_result_result = {
            let schedule = BezierBooleanTraversalScheduleReport2 {
                status: hypercurve::BezierBooleanTraversalScheduleStatus::Ready,
                precondition_status: hypercurve::BezierBooleanTraversalPreconditionStatus::Ready,
            ...
            let ownership_facts = schedule
                ...
                .collect::<Vec<_>>();
            let endpoints = [(p(0, 0), p(0, 0)), (p(1, 0), p(1, 0))];
            let seed = BezierBooleanResultReport2::from_schedule_graph_walk_containment_facts(
                &schedule,
                BooleanOp::Union,
                &ownership_facts,
                &endpoints,
            ...
            let output = BezierBooleanOutputLoopReport2 {
                status: hypercurve::BezierBooleanOutputLoopStatus::Ready,
                closure_status: hypercurve::BezierBooleanLoopClosureStatus::Closed,
                operation: BooleanOp::Union,
                directed_fragments: seed.directed_fragments.clone(),
                loops: seed
                    .assigned_loops
                    .iter()
                    .map(|assigned| assigned.output_loop.clone())
                    .collect(),
                closed_loop_count: seed.assigned_loops.len(),
                directed_fragment_count: seed.directed_fragments.len(),
                open_chain_count: 0,
            ...
            let results = queries
                ...
                .map(|query| BezierBooleanLoopContainmentQueryResult2 {
                    query_loop_index: query.query_loop_index,
                    candidate_container_loop_index: query.candidate_container_loop_index,
                    result: if query.query_loop_index == 1
                        && query.candidate_container_loop_index == 0
                    {
